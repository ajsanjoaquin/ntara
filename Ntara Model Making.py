...
## Builds a sample database containing all of the reactions that we will need in forming the model (which contains reactions
## that are a subset of this sample database)


# In[6]:


metacyc = scobra.db('C:/Users/Wayne/Desktop/Yale NUS stuff/Computational Biology/Files/metacyc/data')
...
sample = BuildModel.BuildRawModelFromDB(metacyc)
...
## Reads the GEN RXN PWY file as a csv file, before filtering out all the reactions (without repetition) in the form of a list


# In[13]:


ntarafile = pandas.read_csv('C:/Users/Wayne/Desktop/Yale NUS Stuff/Computational Biology/Files/NEW DATA/Ntara_2.0_gen_rxn_pwy_20180614',sep='\t')
...
from scobra import BuildModelGRF


# In[18]:


reload(BuildModelGRF)


# In[19]:


ntarafile.to_csv("Ntara.csv", sep = ",")
...
reactionlist = []
for a in ntarafile["Reactions"]:
    if str(a) != '0':
        reactionlist.append(a)
...
for reac in reactionlist:
    BuildModelGRF.ReactionParserExcel(reac, reactions)
...
final = BuildModelGRF.removerepetition(reactions)
...
## Builds the preliminary version of the model based on the reactions found. Done using the submodel function. Then, writes
## the model as an excel xlsx formatted sheet.


# In[29]:


ntarav1 = sample.SubModel(final)
...
ntarav1.WriteModel("NtaraModelV1.xlsx", "excel")


# In[ ]:


## The following code produces appropriate entries for the GPR and Genes Column of the "Reaction List" of the 
## excel workbook, using code from BuildModelGRF.


# In[32]:


NtaraModelReactions = pandas.read_excel('C:/Users/Wayne/NtaraModelV1.xlsx', sheet_name = 0, sep = '\t')
...
NtaraModelReactions = NtaraModelReactions.fillna('')


# In[35]:


genedictionary = BuildModelGRF.GeneFinderFinal(final, ntarafile)
...
BuildModelGRF.eraser(NtaraModelReactions, 'GPR')


# In[38]:


BuildModelGRF.eraser(NtaraModelReactions, 'Genes')


# In[39]:


NtaraReactionSheet = BuildModelGRF.GPRfillerupdated(genedictionary, NtaraModelReactions)
...
## The following code attempts to fill up missing information (Charge, netural formula, charged formula) of as many metabolites
## as possible by referring to both the C3 and soybean models.


# In[43]:


SoybeanReference =pandas.read_excel('C:/Users/Wayne/Desktop/Yale NUS stuff/Computational Biology/Files/soybean.xls', sheet_name = 2, sep = '\t')
...
SoybeanReference = SoybeanReference.fillna(0)


# In[46]:


C3Reference = pandas.read_excel('C:/Users/Wayne/Desktop/Yale NUS stuff/Computational Biology/Files/C3single_v14.xls', sheet_name = 1, sep = '\t')
...
C3Reference = C3Reference.fillna(0)
...
NtaraModelMetabolites =pandas.read_excel('C:/Users/Wayne/NtaraModelV1.xlsx', sheet_name = 1, sep = '\t')


# In[51]:


NtaraModelMetabolites


# In[52]:


NtaraModelMetabolites = NtaraModelMetabolites.fillna('')


# In[53]:


metabolitelist = BuildModelGRF.MetaboliteReturner(NtaraModelMetabolites)
...
metabolitedictionary = BuildModelGRF.MetaboliteInfoReturner(metabolitelist,SoybeanReference)
...
NtaraModelMetabolitesSoybean = BuildModelGRF.ExcelMetaboliteInfoFiller(metabolitedictionary,NtaraModelMetabolites)


# In[59]:


NtaraModelMetabolitesSoybean


# In[60]:


metabolitelistV2 = BuildModelGRF.MetaboliteReturner(NtaraModelMetabolitesSoybean )


# In[61]:


len(metabolitelistV2)


# In[62]:


metabolitedictionaryV2 = BuildModelGRF.MetaboliteInfoReturner(metabolitelistV2, C3Reference)


# In[63]:


metabolitedictionaryV2


# In[64]:


NtaraModelMetabolitesFinal = BuildModelGRF.ExcelMetaboliteInfoFiller(metabolitedictionaryV2,NtaraModelMetabolitesSoybean)


# In[65]:


NtaraModelMetabolitesFinal


# In[66]:


metabolitelistV3 = BuildModelGRF.MetaboliteReturner(NtaraModelMetabolitesFinal )


# In[67]:


len(metabolitelistV3)


# In[69]:


NtaraModelMetabolitesFinal.fillna('') 


# In[70]:


writer = pandas.ExcelWriter('NtaraModelV2.xlsx')
NtaraReactionSheet.to_excel(writer,'Reaction List', index = False)
NtaraModelMetabolitesFinal.to_excel(writer, 'Metabolite List', index = False)
writer.save()
...
NtaraModelFinalVersion = scobra.Model('C:/Users/Wayne/NtaraModelV2.xlsx')


# In[72]:


NtaraModelFinalVersion


# In[6]:


## The following code then compartments the model according to the Compartment Dictionary given. However, reactions that are
## not found in the model itself must first be removed from the compartment dictionary before this can be done.


# In[73]:


CompartmentRefDictionary = {'default' : 'Cytosol','compartments':{'Plastid' : [
...
]}, 'suffix':{'Cytosol':'_c', 'Plastid': '_p', 'Peroxisome': '_x', 'Mitochondria':'_m'}}


# In[74]:


CompartmentRefDictionary


# In[75]:


len(CompartmentRefDictionary['compartments']['Cytosol'])


# In[76]:


CompartmentDictionary = BuildModelGRF.RemoveUnwantedReactionsDictionary(CompartmentRefDictionary, final)


# In[79]:


CompartmentDictionary  = BuildModelGRF.RemoveUnwantedReactionsDictionary(CompartmentDictionary, final)
...
NtaraCompartmented = BuildModelGRF.CompartmentModel(NtaraModelFinalVersion, CompartmentDictionary )
...
## The following code then finds the imbalanced and number of imbalanced reactions that the model has. Then, the code 
## finds reaction stoichiometries of these imbalanced reactions from other reference models (c3 and soybean) and substitutes
## the original stoichiometries with the ones found from the other reference models.


# In[82]:


imbalanced = NtaraCompartmented.ImbalanceReactions()
...
NtaraCompartmented.WriteModel("NtaraUnbalanced.xlsx", "excel")


# In[86]:


SoybeanReferenceReac =pandas.read_excel('C:/Users/Wayne/Desktop/Yale NUS stuff/Computational Biology/Files/soybean.xls', sheet_name = 1, sep = '\t')


# In[87]:


SoybeanReferenceReac


# In[88]:


SoybeanReferenceReac = SoybeanReferenceReac.fillna(0)


# In[89]:


imbalancelist = BuildModelGRF.ImbalanceReactionList(imbalanced)
...
refreac = BuildModelGRF.ReferenceReactionInfo(SoybeanReferenceReac, imbalancelist)
...
# Here, the user can compare the reactions stoichiometries found in the original model and the one found from the reference
# models.
...
ntarareacinfo = BuildModelGRF.CompareImbalance(refreac,'C:/Users/Wayne/NtaraUnbalanced.xlsx' )
...
NtaraUnbalancedFixedReactions = BuildModelGRF.FixImbalance(refreac, 'C:/Users/Wayne/NtaraUnbalanced.xlsx')


# In[96]:


NtaraUnbalancedFixedReactions.fillna('')


# In[97]:


NtaraUnbalancedFixedReactions


# In[98]:


NtaraUnbalancedFixedMetabolites = pandas.read_excel('C:/Users/Wayne/NtaraUnbalanced.xlsx', sheet_name = 1, sep = '\t')


# In[101]:


NtaraUnbalancedFixedMetabolites.fillna('') 


# In[100]:


NtaraUnbalancedFixedMetabolites


# In[107]:


writer = pandas.ExcelWriter('NtaraUnbalancedFixed.xlsx')
NtaraUnbalancedFixedReactions.to_excel(writer,'Reaction List', index = False)
NtaraUnbalancedFixedMetabolites.to_excel(writer, 'Metabolite List', index = False)
writer.save()
...
NtaraUnbalancedFixedModel = scobra.Model('C:/Users/Wayne/NtaraUnbalancedFixed.xlsx')


# In[109]:


NtaraUnbalancedFixedModel 


# In[105]:


newimbalanced = NtaraUnbalancedFixedModel.ImbalanceReactions()
